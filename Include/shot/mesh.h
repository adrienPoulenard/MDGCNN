...
//#include </home/adrien/Documents/cpp_libs/flann-1.8.4-src/src/cpp/flann.hpp>
#endif
...
public:
   ...
   //kd_tree_d* flann_index;

   //flann::Index< flann::L2<double> >* flann_index;
   Eigen::MatrixXd flann_matrix;
   //double* flann_data;
   //flann::Matrix<double>* flann_data_mat;
   kdtree3d<double>* search_tree;
...
public:
   ...
   void update_kd_tree()
   {
		if (search_tree)
	   {
		...
		flann_matrix.resize(vertices.size(), 3);
	
		for (size_t i=0; i<vertices.size(); ++i)
		{
			const oriented_point& p = vertices[i].p;
			flann_matrix(i, 0) = p.x;
			flann_matrix(i, 1) = p.y;
			flann_matrix(i, 2) = p.z;
		}
		...
		//flann_index = new kd_tree_d(flann_matrix, 10); // max leaf = 10
		//flann_index->index->buildIndex();
		search_tree = new kdtree3d<double>(flann_matrix);
   ...
   void put_vertices(const std::vector< vec3d<double> >& points)
	...
	void nearest_neighbors_with_dist(int p, double radius, std::vector<int>& neighs, std::vector<double>& dists)
	{
	   if (!search_tree)
		...
		Eigen::Vector3d q(pt.x, pt.y, pt.z);
		...
		search_tree->radiusSearch(q, radius, ns, ds);
