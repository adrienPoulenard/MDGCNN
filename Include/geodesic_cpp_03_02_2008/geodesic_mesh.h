...
inline void Mesh::build_adjacencies()
{
	...
	{
		Face& f = m_faces[i];
		//std::cout << f.adjacent_vertices().size() << std::endl;
		for(unsigned j=0; j<3; ++j)
		{
	...
	//			Vertices->adjacent Vertices (add)
	
	for (unsigned i = 0; i<m_vertices.size(); ++i)
	{
		m_vertices[i].adjacent_vertices().set_allocation(allocate_pointers(m_vertices[i].adjacent_edges().size()), 
																	       m_vertices[i].adjacent_edges().size());
	}
	for (unsigned i = 0; i < m_vertices.size(); i++) {
		for (unsigned j = 0; j < m_vertices[i].adjacent_edges().size(); j++) {
			if (m_vertices[i].adjacent_edges()[j]->adjacent_vertices()[0]->id() != i) {
				m_vertices[i].adjacent_vertices()[j] =
				m_vertices[i].adjacent_edges()[j]->adjacent_vertices()[0];
			}
			else {
				m_vertices[i].adjacent_vertices()[j] =
				m_vertices[i].adjacent_edges()[j]->adjacent_vertices()[1];
			}
		}
	}

	//			Faces->adjacent Vertices (add)
	for (unsigned i = 0; i<m_faces.size(); ++i)
	{
		m_faces[i].adjacent_vertices().set_allocation(allocate_pointers(3), 3);
	}
	count.resize(m_faces.size());
	std::fill(count.begin(), count.end(), 0);
	for (unsigned i = 0; i < m_vertices.size(); i++) {
		unsigned f_id = 0;
		for (unsigned j = 0; j < m_vertices[i].adjacent_faces().size(); j++) {
			f_id = m_vertices[i].adjacent_faces()[j]->id();
			m_faces[f_id].adjacent_vertices()[count[f_id]] = &m_vertices[i];
			count[f_id]++;
		}
	}
	...
	//			Faces->adjacent Faces (add)
	for (unsigned i = 0; i<m_faces.size(); ++i)
	{
		unsigned int nb_adj_faces;
		nb_adj_faces = 0;
		for (unsigned j = 0; j < 3; j++) {
			if (m_faces[i].adjacent_edges()[j]->adjacent_faces().size() == 2) {
				nb_adj_faces++;
			}
		}
		m_faces[i].adjacent_faces().set_allocation(allocate_pointers(nb_adj_faces), nb_adj_faces);
	}
	...
	for (unsigned i = 0; i < m_edges.size(); i++) {
		unsigned f1_id = 0;
		unsigned f2_id = 0;
		if (m_edges[i].adjacent_faces().size() == 2) {
			f1_id = m_edges[i].adjacent_faces()[0]->id();
			f2_id = m_edges[i].adjacent_faces()[1]->id();
			m_faces[f1_id].adjacent_faces()[count[f1_id]] = &m_faces[f2_id];
			count[f1_id] ++;
			m_faces[f2_id].adjacent_faces()[count[f2_id]] = &m_faces[f1_id];
			count[f2_id] ++;
		}
	}
