...
public:
	...
	SurfacePoint(face_pointer f, double a = 1. / 3., double b = 1. / 3., double c = 1. / 3.) :		//set the surface point in barycentric coordinates of the face
		m_p(f)
	{
		double abc = a + b + c;
		if ((fabs(abc) > std::numeric_limits<double>::min()) && (a>=0.0) && (b >= 0.0) && (c >= 0.0)) {
			a /= abc;
			b /= abc;
			c /= abc;
			double x_ = 0.0;
			...
			double z_ = 0.0;
			for (int i = 0; i < 3; i++) {
				x_ += f->adjacent_vertices()[i]->x();
				y_ += f->adjacent_vertices()[i]->y();
				z_ += f->adjacent_vertices()[i]->z();
			}
			set(a*x_, b*y_, c*z_);
		}
		else {
